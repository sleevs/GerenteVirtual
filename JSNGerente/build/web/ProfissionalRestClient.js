...
// Model for Profissional entity
    models.Profissional = Backbone.Model.extend({
        urlRoot: "http://localhost:8081/JSNGerente/webresources/com.jsnsoftware.gerente.profissional/",
        idAttribute: 'profissionalId',
        defaults: {
            profissionalNome: "",
            profissionalEmail: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('profissionalId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8081/JSNGerente/webresources/com.jsnsoftware.gerente.profissional/';
            }
    ...
    // Collection class for Profissional entities
    models.ProfissionalCollection = Backbone.Collection.extend({
        model: models.Profissional,
        url: "http://localhost:8081/JSNGerente/webresources/com.jsnsoftware.gerente.profissional/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ProfissionalCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-profissional-list-itemis template identifier for item
                        templateName: '#tpl-profissional-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.profissional = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.profissional,
                    // tpl-profissional-details is template identifier for chosen model element
                    templateName: '#tpl-profissional-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Profissional();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-profissional-details is a template identifier for chosen model element
                templateName: '#tpl-profissional-details',
